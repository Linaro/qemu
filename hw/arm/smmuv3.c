...
{
    struct iommu_device_info_smmuv3 *info = &sdev->info;
    uint32_t data_type;
    int ret;
    ...
    if (data_type != IOMMU_DEVICE_DATA_ARM_SMMUV3) {
        error_report( "Wrong data type!");
        return -ENOENT;
    }
                               ...
                               uint8_t tg, uint64_t num_pages, bool leaf)
{
    ...
    event.entry.data_type = IOMMU_PGTBL_TYPE_ARM_S1;
...
static void smmuv3_config_ste(SMMUState *bs, uint32_t sid)
{
    ...
    if (cfg->disabled || cfg->bypassed) {
        iommu_config.config = IOMMU_SMMUV3_CONFIG_BYPASS;
    } else if (cfg->aborted) {
        iommu_config.config = IOMMU_SMMUV3_CONFIG_ABORT;
    } else {
        iommu_config.config = IOMMU_SMMUV3_CONFIG_TRANSLATE;
    }

    trace_smmuv3_config_ste(mr->parent_obj.name,
                            iommu_config.config, cfg->s1ctxptr);

    ret = smmu_iommu_install_nested_ste(bs, sdev, IOMMU_PGTBL_TYPE_ARM_S1,
