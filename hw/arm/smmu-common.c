...
void smmu_iommu_uninstall_nested_ste(SMMUState *s, SMMUDevice *sdev)
{
    ...
    if (hwpt && hwpt->out_fault_fd) {
        hwpt->exiting = true;
        qemu_thread_join(&hwpt->thread);
                                  ...
                                  void *data, void * (*handler)(void *))
{
    ...
    if (hwpt->out_fault_fd) {
        ...
        qemu_thread_create(&hwpt->thread, "fault handler", handler,
...
static void smmu_base_realize(DeviceState *dev, Error **errp)
{
    ...
    if (s->iommufd) {
        ...
        if (s->nested) {
            memory_region_init(&s->stage2, OBJECT(s), "stage2", UINT64_MAX);
            memory_region_init_alias(&s->sysmem, OBJECT(s),
                                     "smmu-sysmem", get_system_memory(), 0,
                                     memory_region_size(get_system_memory()));
            memory_region_add_subregion(&s->stage2, 0, &s->sysmem);

            memory_region_init(&s->root, OBJECT(s), "root", UINT64_MAX);
            memory_region_add_subregion(&s->root, 0, &s->stage2);
        }
    }
